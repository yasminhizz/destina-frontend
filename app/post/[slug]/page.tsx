...
async function getRelatedPosts(categoryId: number, currentPostId: number) {
  const res = await fetch(
    `https://cms.destina.my/wp-json/wp/v2/posts?_embed=author,wp:featuredmedia&categories=${categoryId}&exclude=${currentPostId}&per_page=3`,
    { cache: "no-store" }
  );
...
function getImage(post: any) {
  return (
    ...
    "https://placehold.co/600x400?text=Destina"
  );
}
